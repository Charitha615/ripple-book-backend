...
class FuturePlansRequestFormController extends Controller
{
    ...
    {
        try {
            $Request = FuturePlansRequestForm::find($id);

            if (!$Request) {
                return response()->json([
                    'success' => false,
                    'message' => 'Future Plans Request Form request not found'
                ], 404);
            }
            ...
            AuditAdminLogController::createLog([
                ...
                'entity_area' => 'Future Plans Request Form',
                'description' => "Changed status of Future Plans Request Form request ID: {$id} to '{$status}'",
                'old_values' => json_encode($oldValues),
            ...
            Log::error('Error updating Future Plans Request Form request status: ' . $e->getMessage());
