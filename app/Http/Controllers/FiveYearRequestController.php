...
class FiveYearRequestController extends Controller
{
    ...
    {
        try {
            $Request = FiveYearRequest::find($id);

            if (!$Request) {
                return response()->json([
                    'success' => false,
                    'message' => 'Five Year Request Form request not found'
                ], 404);
            }
            ...
            AuditAdminLogController::createLog([
                ...
                'entity_area' => 'Five Year Request Form',
                'description' => "Changed status of Five Year Request Form request ID: {$id} to '{$status}'",
                'old_values' => json_encode($oldValues),
            ...
            Log::error('Error updating Five Year Request Form request status: ' . $e->getMessage());
