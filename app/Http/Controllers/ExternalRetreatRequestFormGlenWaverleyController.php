...
class ExternalRetreatRequestFormGlenWaverleyController extends Controller
{
    public function store(Request $request)
    {
        try {
            ...
            $externalRetreatRequest = ExternalRetreatRequestForm_GlenWaverley::create([
            ...
            try {
                UserLogController::createLog([
                    'user_id' => null,
                    'form_id' => $externalRetreatRequest->id,
                    'action_type' => 'form_submission',
                    'entity_area' => 'External Retreat Request Form Glen Waverley',
                    'old_values' => null,
                    'new_values' => $externalRetreatRequest,
                    'description' => $validatedData['firstName'] . " submitted an External Retreat Request Form (Glen Waverley). Mobile number is " . $validatedData['mobileNumber'],
                ]);
    ...
    {
        try {
            $externalRetreatRequest = ExternalRetreatRequestForm_GlenWaverley::find($id);

            if (!$externalRetreatRequest) {
                return response()->json([
                    'success' => false,
    ...
    public function edit(Request $request, $id)
    {
        ...
        try {
            ...
            $oldValues = $externalRetreatRequest->getAttributes();
            ...
            AuditAdminLogController::createLog([
                ...
                'new_values' => json_encode($externalRetreatRequest->fresh()->getAttributes()),
            ]);
    ...
    public function getAll(Request $request)
    {
        try {
            ...
            $externalRetreatRequests = ExternalRetreatRequestForm_GlenWaverley::paginate($perPage);

            return response()->json([
                'success' => true,
                'data' => $externalRetreatRequests
            ], 200);
    ...
    {
        try {
            $Request = ExternalRetreatRequestForm_GlenWaverley::find($id);

            if (!$Request) {
                return response()->json([
                    'success' => false,
                    'message' => 'External Retreat Request Form Glen Waverley request not found'
                ], 404);
            }
            ...
            Log::error('Error updating External Retreat Request Form Glen Waverley request status: ' . $e->getMessage());
