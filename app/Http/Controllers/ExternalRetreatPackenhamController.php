...
class ExternalRetreatPackenhamController extends Controller
{
    ...
    {
        try {
            $Request = ExternalRetreatPackenham::find($id);

            if (!$Request) {
                return response()->json([
                    'success' => false,
                    'message' => 'External Retreat Request Form Packenham request not found'
                ], 404);
            }
            ...
            Log::error('Error updating External Retreat Request Form Packenham request status: ' . $e->getMessage());
